...
-- Work with the cylindrical σ-algebra on `WeakDual` generated by evaluations.
...
theorem exists_gaussianProcessWeakDual_of_nuclear
  ...
  have h_integrand_meas :
    ...
    have h_eval : Measurable (fun ω : WeakDual ℝ E => (ω f : ℝ)) := by
      -- We use the cylindrical σ-algebra on `WeakDual` from `MinlosGaussianToWeakDual`.
      have h_toFun : Measurable (MinlosGaussianToWeakDual.toFun (E := E)) :=
        MinlosGaussianToWeakDual.measurable_toFun (E := E)
      simpa [MinlosGaussianToWeakDual.toFun] using (measurable_pi_apply f).comp h_toFun
    have h_mulI : Measurable (fun x : ℝ => (I : ℂ) * (x : ℂ)) :=
      (continuous_const.mul continuous_ofReal).measurable
    have h_exp : Measurable (fun z : ℂ => Complex.exp z) :=
      Complex.continuous_exp.measurable
    exact h_exp.comp (h_mulI.comp h_eval)
  have h_map :
